...
## ADR-0053 — Telegram bot integration: single-tenant frontend over /search/ask + /documents/upload

**Context:** Mnela has two roles: an inbox for raw thought (notes, voice, photos, files dropped in any order) and an Ask Brain that streams answers with citations grounded in the vault. Both are reachable only through `/ask` in the web UI — which is fine at a desk but useless when the thought happens on a walk, in a meeting, or while looking at a paper receipt. The same model already ingests `source='telegram'` documents (the enum value has been in the schema since the original `SourceType` definition, never populated). Every plumbing requirement for an out-of-the-box bot is already in place: `POST /documents/upload` accepts multipart with mime auto-detection; the worker's transcription queue (`apps/worker/src/transcription/`) handles OGG/OPUS audio via whisper.cpp; the orchestrator's vision queue analyzes images and writes `Attachment.description` + `Attachment.ocrText`; `POST /search/ask` streams an agent-loop SSE with `scopeProjectSlug` plumbed through tool context; `Conversation` + `Message` tables manage multi-turn state server-side; the `keystore` from `@mnela/llm-providers` reuses for any AES-256-GCM secret. What is missing is the Telegram-side adapter and a way to configure it without redeploying.

A separate concern is that the user evaluated [OpenClaw](https://openclaw.ai/) (a local task-automation assistant that ships with TG/WhatsApp/Discord transports) and decided not to use it: OpenClaw's focus is **task automation** (read mail, fill forms, run shell commands), while Mnela's focus is **structured knowledge**. The two are orthogonal — the integration described here is _not_ a redundant re-implementation of OpenClaw's Telegram transport; it is the knowledge-base equivalent that OpenClaw explicitly doesn't try to be.

**Decision:** Ship a new NestJS process `apps/tg-bot/` that talks to Telegram via [grammY](https://grammy.dev/) (current support: Bot API 9.6, released 2026-04-03) and treats Telegram as the second canonical client of the existing `/search/ask` + `/documents/upload` endpoints. Configuration is single-tenant, lives in a `TelegramBot` singleton row + `TelegramAllowedUser` whitelist, and is surfaced under a new **Telegram** card in `/admin/system`. The whole feature is gated by `telegram.enabled` so the process can ship dark and be turned on per-environment.

1. **Library and streaming.** grammY is the modern TypeScript-first choice in 2026 — Telegraf's typing migration was painful, many of its plugins are unmaintained (`telegraf-i18n`, `telegraf-inline-menu` moved to grammY). For the reply path we use Bot API 9.5's `sendMessageDraft` (released 2026-03-01) via grammY's `stream` plugin: the user sees characters appear progressively, with no `editMessageText` flood-wait dance. Fallback path on Bot API <9.5 servers: single `sendMessage` after the full SSE drains. Bot API 9.3's `messages.transcribedAudio` is **not** available to bots (MTProto user-account API only), so voice still goes through Mnela's own whisper.cpp pipeline — which is desirable anyway: deterministic, local, no Premium-quota dependency.

2. **Process placement.** New top-level app `apps/tg-bot/` next to `apps/{api,orchestrator,worker,mcp}`. It does **not** live inside the orchestrator (which is BullMQ-consumer-shaped and not concerned with external transports) and **not** inside the api (which would mean a restart per token swap). The bot process holds one long-poll connection per `enabled=true` flip, subscribes to a Redis `mnela:events` pub/sub channel filtering on `telegram:reload`, and restarts grammY cleanly on token/enabled changes — flipping the toggle in `/admin/system` reaches the bot in <1s with no manual restart. `OnApplicationBootstrap` + `OnModuleDestroy` bracket the grammY lifecycle. Internal calls back to `apps/api` use a service-bearer token sourced from `MNELA_INTERNAL_TOKEN` env (separate from the Telegram bot token).

3. **Single-tenant by construction.** Mnela has no `userId` column on `Document` / `Project` / `Entity`; the token IS the owner. The TG bot keeps this contract: there is exactly one `TelegramBot` singleton row (literal `id='singleton'`), one shared whitelist, one shared vault. The whitelist gates access — a TG user not in `TelegramAllowedUser` gets exactly one "❌ not authorized" reply per chat and is silently ignored thereafter. This is intentional: Mnela is a personal second brain, not a multi-tenant service, and bolting `userId`-everywhere on for a single new feature would be far more invasive than the feature is worth. If multi-tenancy is ever required it's a top-down refactor, not a bot-shaped one.

4. **Multi-modal turn bundling.** Telegram delivers messages individually — voice, then photo, then text are three `update` events, not one. The bot maintains a `TurnBuffer<chatId>` keyed by chat id; every incoming update from a whitelisted user (a) sets a 👀 reaction on the source message (b) appends to the chat's pending turn (c) resets a per-chat debounce timer to `telegram.bundleWindowMs` (default 4 s, registry-tunable). When the timer elapses, the bot:
   - uploads each non-text item to `POST /documents/upload` with `source='telegram'` and `metadata.telegram = { chatId, msgId, userId, turnId }` — `turnId` is a fresh cuid shared by every Document created in the same bundle, so the graph reflects "these arrived together"
   - sets reaction 🎧 on voice / 📷 on photo while the worker transcribes/analyses (the bot subscribes to `mnela:events` `document.transcribed` + `document.enriched` to swap the reaction to ✅ when ready)
   - assembles a single user-message string from { transcripts, photo descriptions, raw text } and POSTs `/search/ask` with `conversationId` (from the `TelegramChatLink`) and `scopeProjectSlug` (from the chat's sticky scope or the `TelegramBot.defaultProjectSlug` fallback)
   - streams the SSE response back via `sendMessageDraft`, then sets reaction ✍️ → ✅ on completion

   The bundle window is a deliberate compromise: long enough to catch "voice → photo → caption" sequences that the user thinks of as one thought, short enough that a single sentence feels responsive. Half a second is too snappy (eats the next message); ten seconds is rude. 4 s landed by testing.

5. **Commands as escape hatches.** Three explicit commands, all opt-in to behaviour that would otherwise be ambiguous:
   - **`/scope <project-slug>`** — sticky scope on the chat (`TelegramChatLink.scopeSlug`). Affects every subsequent `/ask` (sets `scopeProjectSlug`) and every ingest (sets `projects: [slug]`). `/scope` with no arg clears it.
   - **`/save <text>`** — force-ingest mode: bypass `/search/ask`, write directly via `mnela_save_note` (synchronous, no LLM agent loop). For "just remember this, don't waste tokens reasoning about it".
   - **`/last [N]`** — show the last N documents the bot saw (default 10), via `mnela_recent_activity` filtered by `source='telegram'`. Fast scroll-back without re-asking.

   A `/forget` command was considered but rejected: too easy to misfire, and the existing `/documents/{id}` UI is the right place for destructive ops.

6. **Reactions as status, not chat noise.** Every status the user needs to see is communicated through Telegram message reactions, not new bot messages: 👀 (received), 🎧 (transcribing audio), 📷 (analysing photo), ✍️ (generating answer), ✅ (done), ❌ (error). This keeps the chat clean — no "I'm thinking…" filler messages — while still giving the live feedback you want when sending a voice note and waiting.

7. **Secret hygiene.** Bot token is AES-256-GCM-encrypted into `TelegramBot.tokenEnc` using the same keystore as `LlmProvider.apiKeyEnc` (`MNELA_PROVIDER_SECRET` env or auto-generated `$MNELA_DATA_DIR/keystore/provider.key`). `TelegramBot.tokenLast4` holds the 4-char suffix so the admin UI can show "…ABCD" without round-tripping decrypt. API responses **never** include `tokenEnc`. `POST /telegram/test` calls grammY's `getMe()` once and caches `botUsername` + `botId` so the UI can show "Connected as @your_bot" without re-decrypting. The token is redacted on every audit log, just like provider keys.

8. **Transport defaults to long-polling.** Webhook is the recommended production setup, but long-polling is the right default for the local-first workflow this project has: no public HTTPS, no tunnel, no inbound firewall hole, no certificate management. Switching to webhook is a config change (`TelegramBot.transport = 'webhook'` + `webhookUrl`) and a process restart — kept available as advanced for when the project actually leaves localhost.

**Alternatives considered:**

- _Use OpenClaw's Telegram transport and expose Mnela as a skill / MCP tool to it._ Inverts ownership: the user would have to install and configure OpenClaw, and every Mnela feature added later would need a skill-side wrapper to be usable in TG. The MCP server at `apps/mcp` already gives any external client (OpenClaw, Claude Code, etc.) access to the read/write tools — so users who DO run OpenClaw can still wire Mnela in that direction without us bundling that path as the default. Rejected as the _primary_ integration: too much external dependency for a feature whose value is "configure one thing, it works."

- _Build the bot inside `apps/orchestrator`._ Tempting because orchestrator already has BullMQ infrastructure and Redis subscriptions. Rejected: orchestrator's responsibility is queue consumption, not external transport. Putting an outbound long-poll loop inside it muddies the deployment model (orchestrator scales for queue load; bot is single-instance). One process per role is cleaner.

- _Auto-classify text as ask-vs-note with a cheap Haiku call._ Adds latency and a token cost on every message, surprises the user when the classifier mispredicts, and complicates the contract. Explicit `/save` prefix is two characters and zero magic.

- _Store the bot token in env (`TELEGRAM_BOT_TOKEN`)._ Doesn't match the project's "everything configurable from /admin/system" pattern, requires a deploy to rotate, and splits the source of truth across env + db. The keystore was built for exactly this.

- _Map each TG user to a separate principal._ Would require a per-user `Conversation` and per-user `source` attribution, and force the schema toward multi-tenancy. The whitelist + shared vault is the honest single-tenant model; if you don't trust whom you whitelist, don't whitelist them.
