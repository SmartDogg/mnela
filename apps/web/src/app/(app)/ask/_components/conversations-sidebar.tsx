...
export function ConversationsSidebar({
  ...
  onNew: () => void;
  /**
   * When 'bare' the component renders without the outer <aside> shell —
   * used by AskSidebar which provides its own pane wrapper.
   */
  chrome?: 'aside' | 'bare';
}): JSX.Element {
  ...
  const bodyNode = (
    <div className="flex-1 overflow-y-auto p-2">
      {list.isLoading && <p className="px-2 py-4 text-xs text-muted-foreground">{t('loading')}</p>}
