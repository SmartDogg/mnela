...
- **"Restart Services" button** (top of `/admin/system`) → `POST /system/restart` → publishes `system.service_reload` on Redis pubsub → each subscriber's `ReloadService` calls its registered hot-reload callbacks. **No `process.exit`** — the consumers close + recreate their BullMQ Workers in-process, so the same one-click flow works under docker / systemd / `pnpm dev` identically. **Subscriber coverage today:** `apps/worker` only. `apps/orchestrator` and `apps/api` do NOT yet subscribe, so `enrichment.parallelism`, `api.rateLimit.*`, and `search.*` weight changes are silently not applied until the corresponding process is killed — tracked as Bucket B in the next-phase plan, plus the honest per-subscriber-ack overlay in PLAN.md Phase 11.
